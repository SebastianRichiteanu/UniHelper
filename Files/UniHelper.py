...
def AddSubWnd():
    ...
    back = Button(root, text="Back", width=width['add_sub'], height=height['top'], font=Font, bg=colors['back'], command=main)
    back.grid(row=0, column=1)
    nameLabel = Label(root, text="Name of the Subject:", background=colors['bg'], fg=colors['label_fg'], font=Font)
...
def EditSubWnd(ind):
    ...
    submit.grid(row=0, column=0)
    back = Button(root, text="Back", width=width['edit_sub'], height=height['top'], font=Font, bg=colors['back'], command=lambda: OpenSub(ind))
    back.grid(row=0, column=1)
    ...
    curNameLabel2 = Label(root, text=l[ind].name, background=colors['bg'], fg=colors['label_fg'], font=Font)
    curNameLabel2.grid(row=1, column=1)
    nameLabel = Label(root, text="New name for the Subject:", background=colors['bg'], fg=colors['label_fg'], font=Font)
...
def RmvSubWnd(ind):
    ...
    canvas_ans = Canvas(root, width=width['full'], height=height["canvas_top"], highlightthickness=0, bg=colors['bg'])
...
def AddHwWnd(ind):
    ...
    submit = Button(root, text="Submit", width=width['add_hw'], height=height['top'], font=Font, bg=colors['submit'],
                    command=lambda ind=ind: AddHwFct(ind, deadlineCal.get_date(), exercText.get(), submitText.get(),
                                                     v1))
    submit.grid(row=0, column=0)
    back = Button(root, text="Back", width=width['add_hw'], height=height['top'], font=Font, bg=colors['back'], command=lambda ind=ind: OpenSub(ind))
    back.grid(row=0, column=1)
...
def EditHwWnd(hw, i, j):
    ...
    if verif_site(hw.exerc):
        exercLabel = Label(root, text=split(hw.exerc), background=colors['bg'], fg=colors['link'], font=Font)
        exercLabel.bind("<Button-1>", lambda e: access_site(hw.exerc))
    else:
        exercLabel = Label(root, text=hw.exerc, background=colors['bg'], fg=colors['label_fg'], font=Font)
    ...
    label_inter1 = Label(root, text="Where to Submit:", background=colors['bg'], fg=colors['label_fg'], font=Font)
    label_inter1.grid(row=4, column=0)

    if verif_site(hw.submit):
        submitLabel = Label(root, text=split(hw.submit), background=colors['bg'], fg=colors['link'], font=Font)
        submitLabel.bind("<Button-1>", lambda e: access_site(hw.submit))
    else:
        submitLabel = Label(root, text=hw.submit, background=colors['bg'], fg=colors['label_fg'], font=Font)
...
def RmvHwWnd(ind, j):
    ...
    canvas_ans.grid_propagate(False)
    canvas_ans.grid(row=1, column=0, pady=30)
    yes = Button(canvas_ans, text="Yes", height=height['mid'], width=width['yes_no'], font=Font, bg=colors['yes'], command=lambda: RmvHwFct(ind, j))
    yes.grid(row=1, column=0, padx=120)
    no = Button(canvas_ans, text="No", height=height['mid'], width=width['yes_no'], font=Font, bg=colors['no'], command=lambda: OpenSub(ind))
...
def OpenSub(ind):
    ...
    clear_window()
    canvas_top = Canvas(root, width=width['full'], height=height['canvas_top'], highlightthickness=0, bg=colors['bg'])
    canvas_top.pack()
    but_AddHw = Button(canvas_top, text="Add A Homework", width=width['open_sub'], height=height['top'], font=Font, bg=colors['add'])
    but_AddHw.configure(command=lambda ind=ind: AddHwWnd(ind))
    but_AddHw.grid(row=0, column=0)
    but_Back = Button(canvas_top, text="Back", width=width['open_sub'], height=height['top'], font=Font, bg=colors['open'], command=main)
    ...
    row = 3
    if len(l[ind].hws) == 0:
        subHwLabel = Label(second_frame, text="No Homeworks ! :)", background=colors['bg'], fg=colors['label_fg'], font=Font)
        ...
        for j in range(len(l[ind].hws)):
            ...
            label_inter1 = Label(canvas_det, text="Subjects:", background=colors['bg'], fg=colors['label_fg'], font=Font)
            label_inter1.grid(row=1, column=0)
            if verif_site(sir2):
                subHwLabel2 = Label(canvas_det, text=sir2, background=colors['bg'], fg=colors['link'], font=Font)
                subHwLabel2.bind("<Button-1>", lambda e: access_site(l[ind].hws[j].exerc))
            else:
                subHwLabel2 = Label(canvas_det, text=sir2, background=colors['bg'], fg=colors['label_fg'], font=Font)
            subHwLabel2.grid(row=2, column=0)

            label_inter1 = Label(canvas_det, text="Where to Submit:", background=colors['bg'], fg=colors['label_fg'], font=Font)
            label_inter1.grid(row=3, column=0)

            if verif_site(sir3):
                subHwLabel3 = Label(canvas_det, text=sir3, background=colors['bg'], fg=colors['link'], font=Font)
                subHwLabel3.bind("<Button-1>", lambda e: access_site(l[ind].hws[j].submit))
            else:
                subHwLabel3 = Label(canvas_det, text=sir3, background=colors['bg'], fg=colors['label_fg'], font=Font)
    ...
    canvas_down = Canvas(root, width=width['full'], height=height['canvas_down'], highlightthickness=0, bg=colors['bg'])
    canvas_down.pack()

    but_Edit = Button(canvas_down, text="Edit This Subject", bg=colors['edit'])
    but_Edit.configure(width=width['open_sub'], height=height['down'], font=Font, command=lambda ind=ind: EditSubWnd(ind))
    but_Edit.grid(row=0, column=0)

    but_Rem = Button(canvas_down, text="Remove This Subject", bg=colors['remove'])
    but_Rem.configure(width=width['open_sub'], height=height['down'], font=Font, command=lambda ind=ind: RmvSubWnd(ind))
...
def main():
    ...
    canvas_main = Canvas(root, width=width['full'], height=height['canvas_half'], highlightthickness=0, bg=colors['bg'])
    canvas_main.pack()
    ...
    if len(l) == 0:
        ...
        firstLabel2 = Label(canvas_main, text="No Subjects added yet", background=colors['bg'])
        firstLabel2.configure(fg=colors['label_fg'], width=width['main'], height=height['mid'], font=Font)
        ...
        for i in range(len(l)):
            ...
            subLabel.configure(background=colors['bg'], fg=colors['label_fg'], font=Font)
            subLabel.grid(row=row, column=0)
            OpenSb = Button(second_frame, text="Open This Subject")
            OpenSb.configure(command=lambda i=i: OpenSub(i), bg=colors['open'], width=width['main'], height=height['mid'], font=Font)
            OpenSb.grid(row=row, column=1)
            row += 1

    canvas_cal = Canvas(root, width=width['full'], height=height['canvas_half'], highlightthickness=0)
    ...
    for ev in ev_color:
        ...
        if ev_color[ev] == 1:
            cal.tag_config(ev, background=colors['homework'], foreground='black')
        elif ev_color[ev] == 2:
